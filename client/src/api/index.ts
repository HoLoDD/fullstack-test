...
class UserAPI {
  ...
  async addUser(username: string, group: IGroup | null) {
    const response = await axios.post<IUser>('API_URL/user', { username });

    if (group) {
      const addResponse = await axios.post<IUser>(`${API_URL}/user/${response.data.id}`, { group_id: group.id })
      return addResponse;
    }
    return response;
  }
  ...
  async editUser(user_id: number, username: string, group: IGroup | null) {
    const response = await axios.patch<IUser>(`#{API_URL}/user/${user_id}`, { username, group });
...
export class GroupAPI {
  ...
  async addGroup(name: string, description: string) {
    const response = await axios.post<IGroup>(`${API_URL}/group`, { name, description });
  ...
  async editGroup(group_id: number, name: string, description: string) {
    ...
    const response = await axios.patch<IGroup>(`${API_URL}/group/${group_id}`, { name, description });
    return response;
  }
}
